...
class _SplashScreenState extends State<SplashScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final hexagonWidth = (size.width * (isTablet ? 0.5 : 0.85)).clamp(
      280.0,
      450.0,
    );
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Positioned.fill(child: const DataStreamParticles()),
...
class _DataStreamParticlesState extends State<DataStreamParticles>
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        return CustomPaint(painter: ParticlePainter(particles: _particles));
      },
    );
  }
}
...
class LogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 1. Outer Hexagonal Shield
    final shieldPath = _createPolygonPath(center, radius, 6, rotation);
    paint.color = AppTheme.accent.withValues(alpha: 0.8 * progress);
    ...
    final glowPaint = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 4.0
      ..color = AppTheme.accent.withValues(alpha: 0.3 * pulse * progress)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8);
    canvas.drawPath(shieldPath, glowPaint);

    // 2. Inner Triple-Circuit Core
    final coreRadius = radius * 0.6;
    for (int i = 0; i < 3; i++) {
      final angleOffset = (i * 2 * math.pi / 3) + (rotation * 1.5);
      final corePath = _createPolygonPath(center, coreRadius, 3, angleOffset);
      paint.color = AppTheme.cyan.withValues(alpha: (0.6 + i * 0.1) * progress);
      paint.strokeWidth = 1.5;
      canvas.drawPath(corePath, paint);

      // Orbital dots
      final dotPos = Offset(
        center.dx + math.cos(angleOffset) * coreRadius,
        center.dy + math.sin(angleOffset) * coreRadius,
      );
    ...
    final pulsePaint = Paint()
      ..color = AppTheme.accent.withValues(alpha: 0.15 * pulse * progress)
      ..style = PaintingStyle.fill;
    canvas.drawCircle(center, coreRadius * 0.4 * pulse, pulsePaint);

    final centerPaint = Paint()
      ..color = AppTheme.accentLight.withValues(alpha: 0.9 * progress)
      ..style = PaintingStyle.fill;
    canvas.drawCircle(center, 4, centerPaint);
  }

  Path _createPolygonPath(
    Offset center,
    double radius,
    int sides,
    double rotation,
    ...
    for (int i = 0; i < sides; i++) {
      final x = center.dx + radius * math.cos(rotation + i * angle);
      final y = center.dy + radius * math.sin(rotation + i * angle);
      if (i == 0) {
        path.moveTo(x, y);
      } else {
        path.lineTo(x, y);
