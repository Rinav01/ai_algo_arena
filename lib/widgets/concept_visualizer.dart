...
class _EnhancedConceptPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    switch (type) {
      ...
      case ConceptType.backtrackingMini:
        ...
        break;
      case ConceptType.puzzleShuffle:
        _drawPuzzle(canvas, size, center);
        break;
  ...
  void _drawDijkstra(Canvas canvas, Size size, Offset center) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        paint..color = AppTheme.accent.withValues(alpha: (1.0 - progress) * 0.5)..strokeWidth = 2;
  ...
  void _drawDFS(Canvas canvas, Size size, Offset center) {
    ...
    canvas.drawCircle(points[visiblePoints], 4, paint..color = AppTheme.accentLight);
  }
  ...
  void _drawAStar(Canvas canvas, Size size, Offset center) {
    ...
    for (double i = 0; i <= t; i += 0.05) {
        double it = i;
        double x = math.pow(1 - it, 2) * start.dx + 2 * (1 - it) * it * control.dx + math.pow(it, 2) * target.dx;
        double y = math.pow(1 - it, 2) * start.dy + 2 * (1 - it) * it * control.dy + math.pow(it, 2) * target.dy;
        path.lineTo(x, y);
    }
    ...
    canvas.drawCircle(start, r, Paint()..style = PaintingStyle.stroke..color = AppTheme.success.withValues(alpha: (1 - r/30)*0.3));
  ...
  void _drawNQueens(Canvas canvas, Size size, Offset center) {
    ...
    List<List<Offset>> frames = [
        [Offset(0, 0)],
        [Offset(0, 0), Offset(1, 1)], // Conflict
        [Offset(0, 0)], // Backtrack
        [Offset(0, 0), Offset(1, 2)],
        [Offset(0, 0), Offset(1, 2), Offset(2, 0)], // Conflict
        [Offset(0, 0), Offset(1, 2)], 
    ];

    final queenPaint = Paint()..style = PaintingStyle.fill;
    final conflictPaint = Paint()..style = PaintingStyle.stroke..strokeWidth = 2..color = AppTheme.error;

    for (var pos in frames[step]) {
       Offset drawPos = Offset(pos.dx * cellSize + cellSize/2, pos.dy * cellSize + cellSize/2);
    ...
    if (step == 1 || step == 4) {
        ...
        Offset p1 = Offset(frames[step][lastIdx-1].dx * cellSize + cellSize/2, frames[step][lastIdx-1].dy * cellSize + cellSize/2);
        Offset p2 = Offset(frames[step][lastIdx].dx * cellSize + cellSize/2, frames[step][lastIdx].dy * cellSize + cellSize/2);
        canvas.drawLine(p1, p2, conflictPaint);
    }
  }

  void _drawPuzzle(Canvas canvas, Size size, Offset center) {
    final cellSize = size.width / 3;
    final boxPaint = Paint()..style = PaintingStyle.fill;
    
    for (int i = 0; i < 3; i++) {
      for (int k = 0; k < 3; k++) {
        if (i == 2 && k == 2) continue; // Hole
        Offset pos = Offset(i * cellSize, k * cellSize);
        double t = animation.value;
        if (i == 2 && k == 1) {
            double slide = math.sin(t * math.pi * 2).clamp(0, 1);
            pos = Offset(pos.dx, pos.dy + slide * cellSize);
        }
        boxPaint.color = AppTheme.accent.withValues(alpha: 0.2 + (i + k) * 0.1);
        canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(pos.dx + 2, pos.dy + 2, cellSize - 4, cellSize - 4), const Radius.circular(4)), boxPaint);
      }
    }
  }
  ...
  void paintDynamicLine(Canvas canvas, Size size, double yFactor, Color color, double t) {
    final paint = Paint()..style = PaintingStyle.stroke..strokeWidth = 3.0..strokeCap = StrokeCap.round..color = color;
    ...
    for (double i = 0.1; i <= progress * 0.8 + 0.1; i += 0.05) {
      path.lineTo(size.width * i, size.height * yFactor + math.sin(i * 10 + t * 5) * 5);
    }
    canvas.drawPath(path, paint);
    canvas.drawCircle(Offset(size.width * (progress * 0.8 + 0.1), size.height * yFactor + math.sin((progress * 0.8 + 0.1) * 10 + t * 5) * 5), 4, paint..style = PaintingStyle.fill);
  }
