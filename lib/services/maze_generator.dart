...
class MazeGenerator {
  ...
  static void generatePrims(
    ...
    while (walls.isNotEmpty) {
      ...
      final unvisited = neighbors
          .where((pos) => controller.grid[pos.r][pos.c].type == NodeType.wall)
          .toList();
    ...
    controller.setNodeType(
      controller.start.row,
      controller.start.column,
      NodeType.start,
    );
  ...
  static void _addPotentialWalls(
    int r,
    int c,
    int rows,
    int cols,
  ...
  static List<({int r, int c})> _getDividedCells(
    int r,
    int c,
    int rows,
    int cols,
